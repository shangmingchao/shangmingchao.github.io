...
- Hash 最常见的应用就是应用于 Hash 表，对于给定的关键词可以快速地定位要查找的那条数据记录，因为 Hash 函数的值就是记录的索引。通常 Hash 函数的定义域大于值域，所以有可能不同的几个 key 会映射到相同的索引，从而出现冲突/碰撞（collision），也正只是因为这样，Hash 表的每个槽可能对应几个记录，这个槽也被称为桶，Hash 值也被称为桶索引  
